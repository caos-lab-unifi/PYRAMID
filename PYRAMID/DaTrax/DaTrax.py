...
def transform(args, transformation, Xtrain, Xtest=None, seed=None):
	...
	if transformation == "scaler":
		...
		try:
			...
			if has_test:
				...
				# use train mean/std to standardize test — no data leakage
...
def run(parser, args):
	...
	if c.VALIDATION:
		...
		try:
			...
			error(f'TSV {c.VALIDATION} does not exist, is not readable or is not a valid TSV: {e}')
			sys.exit(1)
		...
		if c.additional_validation:
			...
			try:
				...
				error(f'TSV {c.additional_validation} does not exist, is not readable or is not a valid TSV: {e}')
				sys.exit(1)
	...
	if c.additional_training:
		...
		try:
			...
			error(f'TSV {c.additional_training} does not exist, is not readable or is not a valid TSV: {e}')
			sys.exit(1)
	...
	if c.VALIDATION:
		...
		X_train_, X_test_ = transform(args, c.transformation, training_, validation_, seed=getattr(args, 'seed', None))
		...
		X_train_ = transform(args, c.transformation, training_, seed=getattr(args, 'seed', None))
		log('Transformation Done')
		log('Writing output')
		X_train_.to_csv(os.path.join(c.OUT, 'train.transformed.tsv'), sep='\t', index=True)
