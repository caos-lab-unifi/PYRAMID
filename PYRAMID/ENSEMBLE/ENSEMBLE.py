...
class TSVEnsemble:
	...
	def get_ensemble_predictions(self) -> np.ndarray:
		...
		if not self.top_predictions:
			...
			error("No valid predictions available for ensemble")
			sys.exit(1)
	...
	def majority_voting(self) -> np.ndarray:

		'''
		Combine top-model binary predictions by majority vote.
		In the case of a tie the ensemble's averaged posterior probability
		is used as the tiebreaker (threshold 0.5).
		Returns a binary array of shape (n_samples,).
		'''

		binary = list(self.top_models_df['binary_predictions'])

		# Step 1: Stack arrays to shape (n_samples, 28)
		stacked = np.stack(binary)

		# Step 2: Count 1s along axis=0
		sum_ones = np.sum(stacked, axis=0)
		n_arrays = stacked.shape[0]
		half = n_arrays / 2

		# Step 3: Initialize result array
		majority_vote = np.zeros(stacked.shape[1], dtype=int)

		# Step 4: Logic
		# Where 1s are majority
		majority_vote[sum_ones > half] = 1

		# Where it's a tie, use fallback array thresholding
		ties = sum_ones == half
		majority_vote[ties] = (self.ensemble_predictions[ties] > 0.5).astype(int)
	...
	def _plot_individual_vs_ensemble(self, ax):
		...
		if len(self.top_predictions) > 10:
			...
			sample_idx = rng.choice(len(self.top_predictions), 10, replace=False)
...
def load_tsv_ensemble(tsv_file: str, score_column: str = 'roc_auc') -> TSVEnsemble:
	...
	try:
		...
		if 'prob' not in df.columns:
			...
			error("'prob' column not found in the file")
			sys.exit(1)
		...
		if score_column not in df.columns:
			...
			for alt in score_alternatives:
				...
				error('No suitable score column found. Available: ' + str(list(df.columns)))
				sys.exit(1)
...
def run(parser,args):
	...
	try:
		...
		error('TSV ' + c.INPUT + ' does not exist, is not readable or is not a valid TSV: ' + str(e))
		sys.exit(1)
	...
	# With true labels (if you have them)
