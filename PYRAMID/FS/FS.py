...
def run(parser,args):
	...
	if not all(item in algorithm_names for item in c.algorithms):
		...
		not_in = str(', '.join(f"{item}" for item in c.algorithms if item not in algorithm_names))
	...
	#Feature Selection
	log('Start feature Selection with ' + str(c.seed) + ' seed, ' + str(', '.join(c.algorithms)) + ' parameter and ' + str(c.threads) + ' threads')
	...
	log('Writing seleted features by ' + str(', '.join(c.algorithms)))
