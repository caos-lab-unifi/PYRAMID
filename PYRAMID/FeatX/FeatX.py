...
def MIScore(Xtrain,ytrain):
	...
	#rankdfMI=rankdfMI[rankdfMI['Score_MI'] > 0]
...
def FilterParameter(dict_, value):

	'''
	Filter ranking-parameter entries whose metric score exceeds a given threshold.
	For each consecutive pair of parameter values, keeps the one whose 75th-percentile
	score is higher. Returns a dict mapping (parameter, n_features) keys to metric
	score Series.
	'''

	keydict=dict()

	for i,k in enumerate(itertools.islice(dict_.keys(), len(dict_) - 1)):

		tmp1 = list(dict_.values())[i]
		tmp2 = list(dict_.values())[i+1]
		
		n_features = tmp1['n_features'].unique()[0]

		# Filter the values greater than
		greater_than = tmp1[metric] >= value

		# Count the number of True values
		count_above = greater_than.sum()

		if count_above >= 3:

			tmp1_3rd = tmp1[metric].quantile(0.75)
			tmp2_3rd = tmp2[metric].quantile(0.75)

			if tmp1[tmp1[metric] > tmp1_3rd].shape[0] > tmp2[tmp2[metric] > tmp2_3rd].shape[0]:

				keydict[(k,n_features)] = tmp1[metric]

			elif tmp1[tmp1[metric] > tmp1_3rd].shape[0] == tmp2[tmp2[metric] > tmp2_3rd].shape[0]: 
...
def SelectDataFrameFromDict(dict_1, dict_2, keylist):

	'''
	From the filtered parameter candidates in keylist, find the entry with the
	highest peak metric score in dict_1, then return the corresponding feature-set
	DataFrame from dict_2.
	'''

	# Create new dataframes based on the keys stored in the list
	selected_dataframes = {key: dict_1[key] for key in keylist}

	# Find the dataframe with the highest value in the specified column
	highest_ = max(selected_dataframes, key=lambda k: selected_dataframes[k].max())

	resdict = dict_2[highest_[0]]

	return resdict


def FindNfeatures(Xtrain, ytrain, FS_rank_df, max_n_features, max_suppAlg, max_rank, threshold, outdir):

	'''
	Search for the optimal feature subset by sweeping over supporting-algorithm
	counts and ranking parameters.
	For each combination, features agreed upon by at least supp algorithms within
	the given rank are evaluated with cross-validation. ALL metrics (accuracy,
	precision, recall, f1, roc_auc) are computed for every classifier.
	Results that exceed the metric threshold and respect the maximum feature budget
	are saved per-supp subdirectory. Returns a dict mapping each supporting-algorithm
	count to the selected feature-set DataFrame.
	'''

	finaldict = {}

	for supp in range(2, max_suppAlg+1):

		suppdir = f"{outdir}/supp{supp}"
		os.makedirs(suppdir, exist_ok=True)
		resdict, FSSdict = {}, {}

		for parameter in range(2, max_rank + 1):

			df_tun = pd.DataFrame(0, index=indexes, columns=estimators, dtype='float64')
			df_tun['parameter'] = parameter

			FSS = FS_rank_df.index[(FS_rank_df <= parameter).sum(axis=1) >= supp].tolist()

			if 2 <= len(FSS) <= FS_rank_df.shape[0]:

				if len(FSS) < max_n_features:

					FSS_df = FS_rank_df[(FS_rank_df <= parameter).sum(axis=1) >= supp].fillna(9999)
					df_tun['n_features'] = FSS_df.shape[0]
					FSSdict[parameter] = FSS_df

					plotHeatmap(FSS_df, FSS_df.isna(), parameter, suppdir)
					FSS_df.to_csv(f"{suppdir}/FSS_dataframe_{parameter}.tsv", sep="\t", index=True)
					...
					for i, alg in enumerate(ML_algorithm):
						...
						for est in estimators:
							...
							try:
								...
								n_scores = cross_val_score(alg, Xtrain[FSS], ytrain, scoring=est, cv=cv, n_jobs=threads)
								log('Cross Validation results for : ' + str(alg) + ' considering ' + str(est) + ': ' + str(np.mean(n_scores)) + '±' + str(np.std(n_scores)))
								df_tun.loc[indexes[i], est] = np.mean(n_scores)

							except Exception as e:

								warn('Cross-validation failed for ' + str(alg) + ' with ' + str(est) + ' at parameter ' + str(parameter) + ' : ' + str(e))

						resdict[parameter] = df_tun

				else:

					warn('For supporting algorithm ' + str(supp) + ' and ranking parameter ' + str(parameter) + ' features are more than ' + str(max_n_features) + ' allowed features')

					break

			else:

				warn('For supporting algorithm ' + str(supp) + ' and ranking parameter ' + str(parameter) + ' features are less than 2')

		if resdict:

			concat = pd.concat(resdict.values())
			concat.to_csv(f"{suppdir}/results.parameters.tsv", sep="\t", index=True)
			PlotViolinFeatures(concat, suppdir)

			pd.concat(FSSdict.values()).to_csv(f"{suppdir}/FSS_dataframe.tsv", sep="\t", index=True)
			tmpdict = FilterParameter(resdict, threshold)
			keylist = [(k, feat) for k, feat in tmpdict.keys() if feat <= max_n_features]

			if keylist:

				finaldict[supp] = SelectDataFrameFromDict(tmpdict, FSSdict, keylist)
				finaldict[supp].to_csv(f"{suppdir}/results.parameters.selected.tsv", sep="\t", index=True)

			else:

				warn(f'No model found with < 20 features for supp {supp} - consider increasing feature limit or decrease f1 threshold')
...
def GetCoeff(reg_name, Xtrain, ytrain, alphas, outdir):
	...
	if reg_class is None:
		...
		error(f"Unsupported regression type: {reg_name}")
		sys.exit(1)
	
	coefficients = {}
	#reg_class = reg_class(random_state=seed)
...
def GridReg(reg_name, Xtrain, ytrain, outdir):
	...
	classifier (Lasso, Ridge, or ElasticNet).
...
def run(parser,args):
	...
	c.rank=args.rank
	c.supporting_algorithm=args.supporting_algorithm
	c.threshold=args.threshold
	c.threads=args.threads
	c.seed=args.seed
	c.maximum=args.n_max_allowed_features
	...
	if not all(item in algorithm_names for item in c.algorithms):

		not_in = [item for item in c.algorithms if item not in algorithm_names]
		error('argument -a/--algorithm: invalid choice: ' + str(not_in) + ' choose from ANOVA, MI, RFECV_LR, RFECV_LR_L1, RFECV_LR_L2, RFECV_LR_EN, RFECV_Perceptron, RFECV_RF, RFECV_GB, RFECV_SVC, Agglomerative, Lasso, ElasticNet, all or a combination of valid arguments')
		sys.exit(1)
	...
	if not c.metadata:
		...
		try:
			...
			error('TSV ' + c.metadata + ' does not exist, is not readable or is not a valid TSV: ' + str(e))
			sys.exit(1)
	...
	if c.additional:
		...
		try:
			...
			error('TSV ' + c.additional + ' does not exist, is not readable or is not a valid TSV: ' + str(e))
			sys.exit(1)
	...
	#Feature Selection
	log('Start feature Selection with ' + str(c.seed) + ' seed, ' + str(c.rank) + ' rank, ' + str(c.supporting_algorithm) + ' supporting algorithm, ' + str(c.threshold) + ' threshold ' + str(c.threads) + ' threads and ' + c.metric + ' scorer')
	...
	if c.algorithms == ['all'] or c.algorithms[0] == 'all':

		algorithm_mapper = AlgorithmMapper(algorithm_names, X_train, y_train, c.OUT)

	else:

		if len(c.algorithms) >= 2:

			algorithm_mapper = AlgorithmMapper(c.algorithms, X_train, y_train, c.OUT)

		else:

			error('the number of algorithm for Feature Selection must be more or equal to 2 to run Feature Selector based on rank')
			sys.exit(1)
	...
	global ML_algorithm
	ML_algorithm = [RandomForestClassifier(random_state=seed), LogisticRegression(penalty='l2',random_state=seed), LogisticRegression(solver='saga',l1_ratio=0.5, penalty='elasticnet',random_state=seed),LogisticRegression(penalty='l1',solver='liblinear',random_state=seed),SVC(kernel='linear', max_iter=100,random_state=seed), SVC(kernel='rbf', max_iter=100,random_state=seed),SVC(kernel='sigmoid', max_iter=100,random_state=seed),GaussianNB(), KNeighborsClassifier(n_neighbors=3), DecisionTreeClassifier(random_state=seed), Perceptron(random_state=seed),GradientBoostingClassifier(random_state=seed)]
	...
	log('Select optimal models with less than ' + str(c.maximum) + ' features')

	#Select optimal models with less features
	selected_features=FindNfeatures(X_train, y_train, FS_rank, c.maximum, c.supporting_algorithm, c.rank, c.threshold, c.OUT)
	
	# Loop through the dictionary and save each DataFrame to a CSV file
	
	if len(selected_features) == 0:

		warn('No feature selected by custom algorithm considering decrease threshold')
		FS_rank.to_csv(c.OUT + '/' + 'FS.results.tsv', sep='\t', index=True,na_rep="NA")

	else:
		
		for key, df in selected_features.items():
		
			log('Writing selected features with ' + str(key) + ' supporting algorithm and ' + str(df.shape[0]) + ' features')
			newdf = pd.DataFrame(1, index=selected_features[key].index, columns=["supporting_algorithm_" + f"{key}"])
			FS_rank = pd.concat([FS_rank,newdf], axis=1, join='outer')
