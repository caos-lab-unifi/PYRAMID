					...
					for i, alg in enumerate(ML_algorithm):
						...
						for est in estimators:
							...
							try:
								...
								error('Cross-validation failed for ' + str(alg) + ' with ' + str(est) + ' at parameter ' + str(parameter) + ' : ' + str(e))
...
def run(parser,args):
	...
	if not all(item in algorithm_names for item in c.algorithms[0].split(',')):
		...
		not_in = [item for item in c.algorithms[0].split(',') if item not in algorithm_names]
	...
	# Ensure n_neighbors does not exceed the training sample size
	global n_neighbors
