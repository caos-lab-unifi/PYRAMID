...
def SearchCV_(Xtrain, ytrain, FSS, RFECV_FS, params, outdir, method='Grid'):
	...
	for i, (k, v) in enumerate(params.items()):
		...
		try:
			...
			if method == 'Grid':
				...
				warn('GridSearchCV tuning failed for ' + str(algorithm_mapper.get_estimators([k])[0]) + ' using ' + str(RFECV_FS) + ' selected features: ' + str(e))
	...
	if (wastedf[metric] != 9999).any():
		...
		wastedf = wastedf[wastedf[metric] != 9999]
...
def run(parser,args):
	...
	#FSdir = os.path.dirname(c.feature_selection)
	...
	for col in FS.columns:
		...
		if FS_:
			...
			if c.search=='GridSearchCV':
				...
				SearchCV_(X_train, y_train, FS_, col, params, c.OUT, method='Grid')

			else:

				# RandomizedSearchCV to find the optimal number of features 
				log('Hyperparameters tuning with ' + c.search + ' ' + metric + ' scorer using ' + str(col))
				SearchCV_(X_train, y_train, FS_, col, params, c.OUT, method='Random')
