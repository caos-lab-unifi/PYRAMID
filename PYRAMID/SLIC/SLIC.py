...
def sampling(Algorithm, samplingStrategy, Xtrain, ytrain, seed, threads, neighbors):

	'''
	Handles the resampling of the dataset using various imbalanced-learn algorithms.
	Caps k_neighbors/n_neighbors to the requested value but reduces it automatically
	if the minority class is too small, logging a warning when this happens.
	'''
	...
	# Cap k to requested value but protect against minority class being too small
	min_class_count = min(Counter(ytrain).values())
	safe_k = max(1, min(neighbors, min_class_count - 1))

	if safe_k < neighbors:

		warn(
			f"Requested k_neighbors={neighbors} exceeds minority class size "
			f"({min_class_count} samples). Reducing to {safe_k}. "
...
def run(parser, args):
	...
	if c.sampling_strategy is not None:
		...
		try:
			...
			X_train_, y_train_ = sampling(c.algorithm, c.sampling_strategy, X_train, y_train, c.seed, c.threads, c.neighbors)
