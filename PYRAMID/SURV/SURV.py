...
def Spearmanfeatures(df):
	...
	"""
	# Remove constant columns first (zero variance)
	df = df.loc[:, df.std() > 0]
...
def EvaluateCoxModel(mdl,Xtrain, Xtest, ytrain, ytest, features,penalization,outdir):
	...
	#retrieve log-hazard ratios (log(HR)) for each feature
	hazard_ratios = np.exp(cph_.coef_)

	coef = np.ravel(cph_.coef_)
	hr = np.exp(coef)  # or however you computed hazard_ratios
...
def penalized_cox_analysis(X_train, y_train, l1_ratio, outdir, label,search):
	...
	# Force X into a pure NumPy array immediately
	X_pure = X_train.to_numpy()
	
	# Force y into a 1D Structured NumPy Array
	# This is the most common cause of the indexing error in survival analysis
	if isinstance(y_train, (pd.DataFrame, pd.Series)):
		
		# convert the DataFrame/Series to a structured array with named fields
	...
	# Process Coefficients (using original X_train for column names)
	...
	# Hyperparameter Tuning with pure NumPy arrays
...
def remove_multicollinearity(df, threshold=5.0):
	...
	# don't remove the constant, just use it for calculation
		...
		if run_aa:
			...
			for model_name, scores_test, scores_train, label in results_to_process:
				...
				try:
					...
					model_aa, high_risk_idx = FitArchetypalModel(Xtrain, scores_train, FS_, n_archetypes=2)
					ArchetypalAnalysis(Xtest, ytest, FS_, outdir, scores_test, label, model_aa, high_risk_idx)
...
def FitArchetypalModel(X_train, y_train, features, n_archetypes=2):

	"""
	Fits AA on features and identifies the high-risk archetype by 
	calculating the Kaplan-Meier median survival for each cluster.
	"""
	# Fit the AA model
	model_aa = AA(n_archetypes=n_archetypes, random_state=seed)
	model_aa.fit(X_train[features].values)
	
	# Get archetype weights and assignments
	alphas_train = model_aa.transform(X_train[features].values)
	assignments = np.argmax(alphas_train, axis=1)
	
	# Extract Time and Event from structured array y_train
	# Usually y_train.dtype.names are ('event', 'time')
	event_col, time_col = y_train.dtype.names[0], y_train.dtype.names[1]
	durations = y_train[time_col]
	events = y_train[event_col]
	...
	for i in range(n_archetypes):
		mask = (assignments == i)
		if mask.any():
			# Use KaplanMeierFitter to find the median survival time of the cluster
			kmf.fit(durations[mask], event_observed=events[mask])
			m = kmf.median_survival_time_
			# If median is not reached, use max duration as proxy for sorting
			medians.append(m if not np.isnan(m) else np.max(durations))
		else:
			medians.append(np.inf)

	# Identify High Risk (Archetype with the SHORTEST median survival)
	high_risk_idx = np.argmin(medians)
	
	log(f"[AA Setup] Median Survival - Arch 0: {medians[0]:.2f} | Arch 1: {medians[1]:.2f}")
	
	return model_aa, high_risk_idx

def ArchetypalAnalysis(Xtest, y_test, features, outdir, riskscores, label, model_aa, high_risk_idx):
	
	"""
	Applies the pre-trained AA model to test data and generates validation plots.
	"""
	
	alphas = model_aa.transform(Xtest[features].values)

	results_df = pd.DataFrame({
		'ptid': Xtest.index,
		'high_risk_weight': alphas[:, high_risk_idx],
		'model_risk_score': riskscores
	}).set_index('ptid')

	results_df['aa_label'] = np.where(results_df['high_risk_weight'] > 0.5, "High Risk", "Low Risk")
